...
export default function TikShan() {
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Outfit',sans-serif", color: "#fff" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .input-box { transition: border-color 0.2s, box-shadow 0.2s; }
        .input-box:focus-within { border-color: #ff2d55 !important; box-shadow: 0 0 0 3px rgba(255,45,85,0.15) !important; }
        .dl-btn { transition: transform 0.15s, background 0.2s, box-shadow 0.2s; cursor: pointer; }
        .dl-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 24px rgba(255,45,85,0.4); }
        .dl-btn:active:not(:disabled) { transform: translateY(0); }
        .dl-btn:disabled { opacity:0.6; cursor:not-allowed; }
        .icon-btn { background:none; border:none; cursor:pointer; color:#888; transition:color 0.15s; }
        .icon-btn:hover { color:#fff; }
        .card { animation: fadeUp 0.4s ease both; background: #141414; border: 1px solid #222; border-radius: 16px; overflow: hidden; }
        .video-chip { display:inline-flex; align-items:center; gap:6px; background:#1e1e1e; border:1px solid #2a2a2a; border-radius:8px; padding:8px 14px; font-size:13px; color:#ccc; text-decoration:none; transition: background 0.15s, border-color 0.15s, color 0.15s; cursor:pointer; }
        .video-chip:hover { background:#252525; border-color:#ff2d55; color:#fff; }
        .video-chip.primary { background: linear-gradient(135deg,#ff2d55,#ff6b35); border-color:transparent; color:#fff; }
        .video-chip.primary:hover { filter:brightness(1.1); border-color:transparent; }
        .stat-pill { background:#1a1a1a; border:1px solid #222; border-radius:999px; padding:5px 12px; font-size:12px; color:#999; display:inline-flex; align-items:center; gap:5px; }
        .steps-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; }
        @media(max-width:600px) { .steps-grid { grid-template-columns:1fr; } .dl-btn span { display:none; } }
      `}</style>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontWeight: 800, fontSize: 20, letterSpacing: "-0.5px" }}>
              Tik<span style={{ color: "#ff2d55" }}>Shan</span>
            </span>
          </div>
        ...
        <div className="input-box" style={{
          ...
          <input
            ...
            style={{
              ...
              color: "#fff", fontSize: 14, fontFamily: "'Outfit',sans-serif", minWidth: 0
            }}
          />
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            ...
            <button className="icon-btn" onClick={handlePaste} style={{
              padding: "8px 12px", fontSize: 12, fontWeight: 600,
              color: "#888", borderRadius: 8, border: "1px solid #222", background: "#161616"
            }}>
              Pegar
            </button>
            <button className="dl-btn" onClick={handleDownload} disabled={loading} style={{
              ...
              {loading ? <><SpinnerIcon /><span>Buscando...</span></> : <><DownloadIcon /><span>Descargar</span></>}
            </button>
          </div>
        </div>
            ...
            <div style={{ display: "flex", gap: 0, flexWrap: "wrap" }}>
              {result.cover && (
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <img src={result.cover} alt="thumbnail"
              ...
              <div style={{ flex: 1, padding: "20px", minWidth: 200 }}>
                {result.author && (
                ...
                {result.title && (
                  <p style={{
                    fontSize: 13, color: "#aaa", lineHeight: 1.5, marginBottom: 14,
            ...
            <div style={{ borderTop: "1px solid #1e1e1e", padding: "20px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                {(result.video?.noWatermark || result.links?.play) && (
                  <a className="video-chip primary" href={result.video?.noWatermark || result.links?.play} target="_blank" rel="noreferrer" download>
                    <VideoIcon /> Video sin marca de agua
                  </a>
                )}
                {result.video?.HD && (
                  <a className="video-chip" href={result.video.HD} target="_blank" rel="noreferrer" download>
                    <VideoIcon /> Video HD
                  </a>
                )}
                {result.video?.watermark && (
                  <a className="video-chip" href={result.video.watermark} target="_blank" rel="noreferrer" download>
                    <VideoIcon /> Con marca de agua
                  </a>
                )}
                {(result.music || result.links?.music) && (
                  <a className="video-chip" href={result.music?.play || result.links?.music} target="_blank" rel="noreferrer" download>
                    <MusicIcon /> Audio MP3
                  </a>
                )}
              </div>
              <p style={{ fontSize: 11, color: "#444", marginTop: 12 }}>
                💡 Si el botón abre el video en vez de descargarlo, haz clic derecho → "Guardar enlace como…"
              </p>
            </div>
          </div>
        )}
            ...
            <div className="steps-grid">
              {[
                ...
                <div key={s.n} style={{ background: "#0f0f0f", border: "1px solid #1a1a1a", borderRadius: 12, padding: "20px" }}>
                  <div style={{ fontFamily: "monospace", fontSize: 11, color: "#ff2d55", fontWeight: 700, letterSpacing: "1px", marginBottom: 10 }}>{s.n}</div>
      ...
      <footer style={{ borderTop: "1px solid #111", padding: "20px 24px", textAlign: "center" }}>
        <p style={{ fontSize: 12, color: "#333" }}>
          TikShan no almacena ningún video. Los archivos provienen directamente de los servidores de TikTok.
        </p>
      </footer>
    </div>
  );
